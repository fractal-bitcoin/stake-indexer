...
const (
	kAll        int = iota // total bucket index
	kThisSecond     = iota // current-second bucket index
	kLastSecond     = iota // previous-second bucket index
	kThisMinute     = iota // current-minute bucket index
	kLastMinute     = iota // previous-minute bucket index
	kNum            = iota
		...
		for _, durIdx := range enabledMetricsDurationDump {
			...
			if data.v[durIdx].latency == 0 {
				if time.Now().After(data.v[durIdx].expiredTime) {
					data.v[durIdx].value = 0 // cleanup expired counter data
					break
				}
...
// flushData rotates time-windowed metric buckets.
...
// Set sets a counter value with an expiration duration.
...
// Inc increments a counter without custom expiration.
...
// Dec decrements a counter without custom expiration.
func (sm *serviceMetrics) Dec(serviceName, serviceType string) {
	sm.addOrSet(serviceName, serviceType, true, -1, modifiedDuration[kAll])
}

// Observe records a request and its latency.
...
func (sm *serviceMetrics) Update(serviceName, serviceType string, latencyDuration time.Duration) {
	...
	if !ok {
		...
		itemIdx = nextItemIdx
		// initialize metric window timestamps
		for _, durIdx := range enabledMetricsDuration {
		...
		// rotate buckets before writing new values
		sm.flushData(itemIdx)
	}
